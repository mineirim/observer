...
Ext.define('ExtZF.controller.plano.Programacoes', {
    ...
    stores: ['programacoes.TreeStore', 'Programacoes', 'Setores', 'Usuarios', 'Instrumentos', 'Operativos',
        'Vinculos', 'Financeiro', 'GrupoDespesas', 'OperativosHistorico', 'anexos.ProgramacaoAnexosStore',
        'Projetos', 'Indicadores', 'IndicadorOpcoes', 'Sistemas'
    ],
    models: ['programacoes.Model4tree', 'Programacoes', 'Setores', 'Usuarios', 'Instrumentos', 'Operativos',
        'Vinculos', 'Financeiro', 'GrupoDespesas', 'OperativosHistorico', 'anexos.ProgramacaoAnexosModel',
        'Projetos', 'Indicadores', 'IndicadorOpcoes', 'Sistemas'
    ],
    views: [
    ...
    itemContextMenu: function(view, record, item, index, event, options) {
        ...
        if (record.get('parentId') === null)
            return;
        var instrumento_filho = me.getStore('Instrumentos').findRecord('instrumento_id', record.get(
            'instrumento_id'));
        var items = [];
        ...
        if (mycontroller.rootNodeSelected) {
            try {
                var rootInstrumento = me.getStore('Instrumentos').findRecord('instrumento_id', rootRecord.get(
                    'instrumento_id'));
                items.push({
    ...
    showReport: function(record, reportType) {
        ...
        window.open("relatorio/index/index/id/" + record.get('id') + filterProjeto + reportType,
            "relatório");
    },
    novaProgramacao: function(parent) {
        ...
        if (typeof(parent) !== 'undefined') {
            ...
            if (parent_id.toString().split('-').length > 1) {
                var instrumento = me.getStore('Instrumentos').findRecord('instrumento_id', parent_id.split(
                    '-')[1]);
                ...
                store.load(function() {
                    ...
                    if (programacao !== null && programacao.get('locked') && !me.checkPermission(
                            programacao)) {
                        Ext.Msg.alert('Atenção', 'Você não tem permissão para criar novo registro!');
                        return;
                    }
                    options.programacao_id = parent_id;
                    var instrumento = me.getStore('Instrumentos').findRecord('instrumento_id',
                        parent.get('instrumento_id'));
                    options.instrumento_id = instrumento.get('id');
                    var view = Ext.widget('planoProgramacoesEdit');
        ...
        if (instrumento.get('has_operativo') === "true") {
            ...
            if (record) {
                ...
                operativoStore.load(function() {
                    var operativo = me.getStore('Operativos').findRecord('programacao_id', parseInt(
                        record.get('id'), 10));
                    if (!operativo) {
                        var rec = {};
    ...
    editarProgramacao: function(rec) {
        ...
        store.load(function() {
            ...
            view.down('form').loadRecord(record);
            var instrumento = me.getStore('Instrumentos').findRecord('id', record.get(
                'instrumento_id'));
    ...
    saveObject: function(button, closes) {
        ...
        if (formDefault.isValid()) {
            ...
            var instrumento = me.getStore('Instrumentos').getById(parseInt(recordSelected.get(
                'instrumento_id'), 10));
            recordSelected.save({
                success: function(a, b) {
                    ...
                    if (check_programacao) {
                        if (instrumento.get('has_vlr_programado') === "true" &&
                            ...
                            me.application.fireEvent('filterDespesasByProgramacao',
        ...
        store.load(function() {
            ...
            if (record.isRoot()) {
                ...
                if (record.get('id') > 0) {
                    ...
                    instrumento = me.getStore('Instrumentos').findRecord('instrumento_id',
                        rootRecord.get('instrumento_id'));
                } else if (rootId.split('-').length > 1) {
                    instrumento = me.getStore('Instrumentos').findRecord('instrumento_id', rootId.split(
                        '-')[1]);
                }
            } else {
                instrumento = me.getStore('Instrumentos').findRecord('instrumento_id', record.get(
                    'instrumento_id'));
            }
            ...
            if (record.get('instrumento').has_operativo) {
                ...
                operativoStore.remoteFilter = true;
                operativoStore.load(function() {
                    var operativo = me.getStore('Operativos').findRecord('programacao_id',
                        parseInt(record.get('id'), 10));
                    if (operativo) {
                        btnExecucao.show();
                        btnExecucao.value = operativo.get('id');
                        me.application.fireEvent('filtrarHistoricoPorOperativo', operativo.get(
                            'id'));
            ...
            me.application.fireEvent('planoProgramacaoFinanceiro.filterByProgramacao', record.get(
                'id'));
    ...
    clickOnDetailsButton: function(button, event) {
        ...
        if (button.action === "execucao") {
            ...
            if (typeof(button.programacao) === 'undefined') {
                ...
                var record = controller.getStore('Operativos').findRecord('programacao_id', programacao.get(
                    'id'));
            }
